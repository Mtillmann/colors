...
var ColorLookup = class {
  ...
  cachedChromaByRGB(r, g, b) {
    ...
    if (!this.chromaCache.has(key)) {
      this.chromaCache.set(key, chroma2(r, g, b));
    }
  ...
  lookupByRGB(r, g, b, maxResults) {
    ...
    for (const color of foundColors) {
      ...
      const deltaE = chroma2.deltaE(
        chroma2.lab(givenLAB[0], givenLAB[1], givenLAB[2]),
        chroma2.lab(
          actualColor.L,
          actualColor.A,
          actualColor.B
    ...
    colors = colors.map((c) => {
      c.euclideanDistance = chroma2.distance(c.hex, givenHex, "rgb");
      return c;
    });
...
export {
  AltShadeLookup_default as AltShadeLookup,
  ColorLookup,
  ShadeLookup
};
